...
```
```{r}
fang_et_al_genotypes <- read.delim(file = "fang_et_al_genotypes")
```
...
```{r}
fang_genotypes_maize <- fang_et_al_genotypes[fang_et_al_genotypes$Group == 'ZMMIL' | fang_et_al_genotypes$Group == 'ZMMLR' | fang_et_al_genotypes == 'ZMMMR', ]
```
```{r}
fang_genotypes_teosinte <- fang_et_al_genotypes[fang_et_al_genotypes$Group == 'ZMPBA' | fang_et_al_genotypes$Group == 'ZMPIL' | fang_et_al_genotypes == 'ZMPJA', ]
...
```{r}
headerless_trans_teo <- transposed_fang_teosinte[-c(1,2,3), ]
```
```{r}
headerless_trans_maize <- transposed_fang_maize[-c(1,2,3), ]
```
